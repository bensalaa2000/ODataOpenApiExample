...
public class ProductsController : ODataController
{
    ...
    {
        ODataPathSegment[] segments = Request.ODataFeature().Path.ToArray();
        Microsoft.OData.Edm.IEdmEntitySet entitySet = ((EntitySetSegment)segments[0]).EntitySet;
        Microsoft.OData.Edm.IEdmNavigationProperty property = entitySet.NavigationPropertyBindings.Single(p => p.Path.Path == navigationProperty).NavigationProperty;

        segments[^1] = new NavigationPropertySegment(property, entitySet);

        Uri relatedKey = new Uri(Request.CreateODataLink(segments));
