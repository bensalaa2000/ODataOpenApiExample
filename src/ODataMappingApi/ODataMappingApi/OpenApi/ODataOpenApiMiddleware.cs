...
public class ODataOpenApiMiddleware
{
    ...
    {
        Microsoft.OpenApi.Models.OpenApiDocument doc = OpenApiDocumentExtensions.CreateDocument(context, prefixName);
    ...
    private void Initialize(ODataOptions options)
    {
        ...
        foreach (KeyValuePair<string, (Microsoft.OData.Edm.IEdmModel EdmModel, IServiceProvider ServiceProvider)> model in options.RouteComponents)
        {
    ...
    private bool TryGetPrefixName(HttpContext context, out string prefixName)
    {
        ...
        foreach (KeyValuePair<TemplateMatcher, string> item in _templateMappings)
        {
